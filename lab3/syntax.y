...
static char *translate_exp(ASTNode *exp) {
    ...
    if (exp->child_count == 3) {
        ...
        if (op->type == NODE_ASSIGNOP) {
            ...
            if (ir_exp_is_id(exp->children[0]) && !ir_is_aggregate(ir_exp_type(exp->children[0]))) {
...
void translate_ir(ASTNode *root_node, const char *out_path) {
    ...
    var_no = 1;
    cannot_translate = 0;
    if (root_node && root_node->child_count > 0) translate_extdeflist(root_node->children[0]);
    fclose(ir_out);
    ir_out = NULL;
    if (cannot_translate) {
